...
BEGIN TRY
      IF ( @tenNguoiDung = null)
          PRINT N'Vui lòng nhập TenND'
      Else IF ( @gioiTinh = null)
          PRINT N'Vui lòng nhập gioi tinh'
      ELSE IF ( @dienThoai = null)
          PRINT N'Vui lòng nhập dien thoai'
      ELSE IF ( @diaChi = null)
          PRINT N'Vui lòng nhập địa chỉ'
      ELSE IF ( @quan = null)
          PRINT N'Vui lòng nhập quận'
      ELSE IF ( @email = null)
...
ELSE IF not exists (select * from NGUOIDUNG WHERE QUAN = @quan)
...
-- NGUOIDUNG) thỏa mãn các giá trị được truyền vào tham số.
	...
	go
	SELECT *
FROM fn_timKiem(DEFAULT,DEFAULT,DEFAULT,DEFAULT,DEFAULT,DEFAULT)
...
--NHATRO). Hàm này trả về tổng số LIKE và DISLIKE của nhà trọ này
	...
	Begin
		...
		values (0,0);
				...
				update @table
				set LIKES = (
						select count(likeornot) from DANHGIA where Manhatro like @Mant and likeornot =1
				)
				update @table
				set DISLIKES = (
						select count(likeornot)from DANHGIA where Manhatro like @Mant and likeornot =0
				)
